...
class BookingModel(Base):
    ...
    - PRIMARY: in-memory AvailabilityIndex (O(1) slot check) is the fast path;
      it rejects conflicts before any DB write.
    - SECONDARY (production PostgreSQL only): a partial unique index on
      (date, time) WHERE NOT cancelled is created by Alembic migration 001.
      This prevents race conditions between concurrent requests that both pass
      the in-memory check. SQLite (used in tests) does not enforce this index
      via create_all, so the in-memory layer is the sole guard in tests.
    """
